...
class ScreenRotate(object):
    """Flat screen for beam visualization."""

    def __init__(self, bl=None, name='', center=[0, 0, 0], x='auto', z='auto',
                 compressX=None, compressZ=None, angle=None, rotationaxis=None):
        """
        *bl*: instance of :class:`~xrt.backends.raycing.BeamLine`
            Container for beamline elements. Optical elements are added to its
            `screens` list.
        ...
        *compressX, compressZ*: float
        ...
        if self.angle is not None and self.rotationaxis is not None:
...
#        self.update_orientation_quaternion()
    ...
    def rotate(self):
        """
        Rotates the screen around a given axis by a specified angle in radians.
        ...
        # Apply the rotation to the orientation vectors if they are defined
        if self._x is not None:
            self._x = np.dot(rot_matrix, self._x)
        if self._z is not None:
            self._z = np.dot(rot_matrix, self._z)
    
        # Recalculate the orthogonal y-vector from the new x and z
        self._set_orientation()
